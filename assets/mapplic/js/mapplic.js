...
;(function($) {
	...
	var Mapplic = function(element) {
		...
		self.init = function(options) {
			...
			self.el.addClass('mapplic-element mapplic-loading').addClass(self.o.skin);
			if (!isNaN(self.o.height)) self.el.height(self.o.height);
		...
		function Sidebar() {
			...
			this.init = function() {
				...
				this.el = $('<div><h3>Key amenity</h3></div>').addClass('mapplic-sidebar').appendTo(self.el);
		...
		var addControls = function() {
			...
			// Double click
			$(document).on('dblclick', '.mapplic-map-image', function(e) {
				e.preventDefault();
		...
		self.showLocation = function(id, duration, check) {
			$.each(self.data.levels, function(index, level) {
				...
				$.each(level.locations, function(index, location) {
					if (location.id == id) {
						var action = typeof location.action !== 'undefined' ? location.action : self.o.action;
						switch (action) {
							case 'open-link':
